...
class BaseUbl(models.AbstractModel):
    ...
    def _ubl_add_address(self, partner, node_name, parent_node, ns, version="2.1"):
        ...
        if partner.building_no:
            building_no = etree.SubElement(address, ns["cbc"] + "BuildingNumber")
            building_no.text = partner.building_no
        if partner.plot_id:
            plot_id = etree.SubElement(address, ns["cbc"] + "PlotIdentification")
            plot_id.text = partner.plot_id
        if partner.city:
    ...
    def _ubl_get_party_identification(self, commercial_partner):
            """This method is designed to be inherited in localisation modules
            Should return a dict with key=SchemeName, value=Identifier"""
            schema_name = commercial_partner.schema_id
            return {
                schema_name:commercial_partner.schema_id_no
    ...
    def _ubl_add_delivery(self, delivery_partner, parent_node, ns, version="2.1"):
        ...
        if self.move_type not in  ('in_invoice','in_refund'):
            delivery_date = etree.SubElement(delivery, ns["cbc"] + "ActualDeliveryDate")
            delivery_date.text = str(self.invoice_date_due)
    ...
    def _ubl_add_tax_category(
        ...
        if not tax:
            tax_exemption = etree.SubElement(tax_category, ns["cbc"] + "TaxExemptionReasonCode")
            tax_exemption.text = "VATEX-SA-HEA"
            tax_exemption = etree.SubElement(tax_category, ns["cbc"] + "TaxExemptionReason")
            tax_exemption.text = "Private healthcare to citizen"
    ...
    # versione = "1.0"
    # xsi: schemaLocation = "http://www.fatturapa.gov.it/sdi/messaggi/v1.0 MessaggiTypes_v1.0.xsd http://www.w3.org/2000/09/xmldsig# xmldsig-core-schema.xsd"
